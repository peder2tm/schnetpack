...
def load_dataset(path, environment_provider, collect_triples,
                 property_map):
    if not os.path.exists(path):
        download()

    data = dat.AtomsData(path, required_properties=property_map.values(),
...
@dataset_ingredient.capture
def download_qm9(path, remove_uncharacterized):
